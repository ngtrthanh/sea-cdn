(function() {
    ...
    function processChunk(chunk) {
        ...
        chunk.forEach(item => {
            ...
            if (lat !== null && lon !== null) {
                const markerKey = MMSI.toString();
                newMarkers.set(markerKey, item);

                if (!currentMarkers.has(markerKey)) {
                    addMarker(item);
                } else {
                    const currentMarkerData = currentMarkers.get(markerKey);
                    if (currentMarkerData.lat !== lat || currentMarkerData.lon !== lon || currentMarkerData.heading !== heading || currentMarkerData.speed !== speed) {
                        updateMarker(item);
                    }
                }
            }
        ...
        newMarkers.forEach((value, key) => {
            currentMarkers.set(key, value);
        });
    }
    ...
    function addMarker(item) {
        ...
        currentMarkers.set(MMSI.toString(), { marker, lat, lon, heading, speed });
    }

    function updateMarker(item) {
        const [MMSI, lat, lon, heading, speed, shiptype, shipname, country] = item;
        const markerData = currentMarkers.get(MMSI.toString());
    ...
    function removeMarker(key) {
        const markerData = currentMarkers.get(key);
        if (markerData && markerData.marker) {
            markerClusterGroup.removeLayer(markerData.marker);
        }
