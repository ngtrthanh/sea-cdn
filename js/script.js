...
// add resetMap button
    ...
    jQuery('#resetMap').on('click', function() {
        jQuery('#resetMap');
    });    
...
// new Page Title update function
function refreshPageTitle() {
    if (pTracks || (!PlaneCountInTitle && !MessageRateInTitle)) return;

    let subtitle = PlaneCountInTitle ? `✈️:${(globeIndex ? globeTrackedAircraft : `${TrackedAircraftPositions}/${TrackedAircraft}`).toLocaleString()}` : '';
    
    if (MessageRateInTitle && MessageRate != null) {
        subtitle += subtitle ? ' | ' : '';
        const msgRate = MessageRate >= 1E06 ? `${(MessageRate / 1E06).toFixed(1)}M` : MessageRate >= 1E03 ? `${(MessageRate / 1E03).toFixed(1)}k` : MessageRate.toFixed(0);
        subtitle += `📨:${msgRate}/s`;
    }
...
(function (global, jQuery, TAR) {
    ...
    planeMan.refresh = function () {
        ...
        jQuery('#dump1090_total_history').text(TrackedHistorySize.toLocaleString());

        //jQuery('#dump1090_message_rate').text(MessageRate === null ? 'n/a' : MessageRate.toFixed(0));

        const messageRateElement = jQuery('#dump1090_message_rate');
        if (MessageRate === null) {
            messageRateElement.updateText('n/a');
        } else {
            const formattedRate = MessageRate >= 1E06
                ? `${(MessageRate / 1E06).toFixed(1)}M`
                : MessageRate >= 1E03
                    ? `${(MessageRate / 1E03).toFixed(1)}k`
                    : MessageRate.toFixed(0);
            messageRateElement.updateText(formattedRate);
        }      

       //jQuery('#dump1090_total_ac').updateText(globeIndex ? globeTrackedAircraft : TrackedAircraft);

        jQuery('#dump1090_total_ac').text((globeIndex ? globeTrackedAircraft : TrackedAircraft).toLocaleString(undefined, { minimumFractionDigits: 0 }));

        jQuery('#dump1090_total_ac_positions').text(TrackedAircraftPositions.toLocaleString(undefined, { minimumFractionDigits: 0 }));
